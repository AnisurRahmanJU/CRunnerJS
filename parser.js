...
// === ADDITION: Support float, double variables and normal functions except main and factorial ===
const originalRunCode = runCode;
runCode = function() {
  let cCode = editor.getValue();

  // Convert float and double variable declarations (arrays removed)
  cCode = cCode.replace(/\b(float|double)\b\s+([^;]+);/g, (m, t, v) => {
    let vars = v.split(',')
      .map(x => x.replace(/\[\d*\]/g, '').trim())
      .join(', ');
    return 'let ' + vars + ';';
  ...
  // Convert function declarations except main and factorial (leave those untouched)
  cCode = cCode.replace(/\b(int|float|double)\s+(\w+)\s*\(([^)]*)\)\s*\{/g, (match, retType, funcName, params) => {
    if (funcName === 'main' || funcName === 'factorial') {
      return match;
    }
    let paramNames = params.split(',')
      .map(p => p.trim().split(' ').pop())
